...
class HwAdapter extends BaseAdapter {
    ...
    public ArrayList<Event>  date_collection_arr;
    private String gridvalue;
    private ListView listTeachers;
    private ArrayList<Dialogpojo> alCustom=new ArrayList<Dialogpojo>();

    public HwAdapter(Activity context, GregorianCalendar monthCalendar,ArrayList<Event> date_collection_arr) {
    ...
    public void setEventView(View v,int pos,TextView txt){
        ...
        int len=Event.date_collection_arr.size();
        for (int i = 0; i < len; i++) {
            Event cal_obj=Event.date_collection_arr.get(i);
    ...
    public void getPositionList(String date,final Activity act){
        ...
        for (int j=0; j<len; j++){
            if (Event.date_collection_arr.get(j).date.equals(date)){
                HashMap<String, String> maplist = new HashMap<String, String>();
                maplist.put("hnames",Event.date_collection_arr.get(j).name);
                maplist.put("hsubject",Event.date_collection_arr.get(j).subject);
                maplist.put("descript",Event.date_collection_arr.get(j).description);
